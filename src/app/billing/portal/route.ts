...
export async function GET(request: NextRequest) {
  try {
    ...
    if (currentRow?.billing_exempt === true) {
      return NextResponse.redirect(new URL(withAppBasePath("/settings?billing=admin_access"), request.url));
    }
    ...
    if (!customer) {
      return NextResponse.redirect(new URL(withAppBasePath(`/settings?billing=${hasAppAccess ? "portal_unavailable" : "missing_customer"}`), request.url));
    }
    ...
    if (!hasStripeSubscription) {
      return NextResponse.redirect(new URL(withAppBasePath("/settings?billing=portal_unavailable"), request.url));
    }

    const portal = await stripe.billingPortal.sessions.create({
      customer: customer.id,
      return_url: `${request.nextUrl.origin}${withAppBasePath("/settings")}`,
    });

    return NextResponse.redirect(portal.url);
  } catch (error) {
    console.error("[billing/portal] failed", error);
    return NextResponse.redirect(new URL(withAppBasePath(`/settings?billing=${portalErrorState(error)}`), request.url));
  }
}
