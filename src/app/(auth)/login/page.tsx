...
function LoginInner() {
  ...
  async function onGoogle() {
    setError(null);
    if (!cloudAuthReady) {
      setError("Google sign-in is disabled until a valid Supabase URL and anon key are set in .env.local.");
      return;
    }
    setLoading(true);
    try {
      ...
      <AuthPanel>
        ...
        {!cloudAuthReady && (
          <p className="mt-5 rounded-xl border border-primary/30 bg-primary/10 p-3.5 text-sm font-medium leading-snug text-zinc-100">
            Google sign-in isn’t configured for this local app. Set a valid Supabase URL and anon key in{" "}
        ...
        <div className="mt-8">
          <GoogleOAuthButton
            ...
            onClick={onGoogle}
            label={cloudAuthReady ? "Continue with Google" : "Google sign-in unavailable"}
          />
        </div>
