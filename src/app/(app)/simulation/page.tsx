    ...
    try {
      ...
      const res = await fetch(`${basePath}/api/python/simulation`, {
        ...
        body: JSON.stringify({
          ...
          targetPrice: trackedStock?.targetPrice,
              ...
              <div className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-xl border border-border/70 bg-background/50 p-3">
                    ...
                    <div className="mt-1 text-xl font-semibold text-foreground">{result.result.periodDescription}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-background/50 p-3">
                  ...
                  <div className="rounded-xl border border-border/70 bg-background/50 p-3">
                    <div className="text-xs uppercase tracking-[0.18em] text-subtle">Win Rate</div>
                    <div className="mt-1 text-xl font-semibold text-foreground">{formatPct(result.result.winRate)}</div>
                  </div>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl border border-border/70 bg-background/50 p-3 text-sm text-subtle">
                    Buys {result.result.buySignals} • Sells {result.result.sellSignals} • Avg profit/trade {formatCurrency(result.result.avgProfitPerTrade)}
                  </div>
                  <div className="rounded-xl border border-border/70 bg-background/50 p-3 text-sm text-subtle">
                    Start {formatCurrency(result.result.startPrice)} • End {formatCurrency(result.result.endPrice)} • Max drawdown {formatPct(result.result.maxDrawdown)}
                  </div>
                </div>

                {result.criteriaUsed ? (
                  <div className="space-y-3 rounded-xl border border-border/70 bg-background/50 p-4">
                    <div className="text-xs uppercase tracking-[0.18em] text-subtle">Criteria Used</div>
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                      <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                        <div className="text-xs uppercase tracking-[0.16em] text-subtle">Buy@Moving Avg</div>
                        <div className="mt-1 text-base font-semibold text-foreground">{result.criteriaUsed.shortSMA} days</div>
                      </div>
                      <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                        <div className="text-xs uppercase tracking-[0.16em] text-subtle">Rebuy Factor</div>
                        <div className="mt-1 text-base font-semibold text-foreground">{result.criteriaUsed.dynamicFactor.toFixed(2)}%</div>
                      </div>
                      <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                        <div className="text-xs uppercase tracking-[0.16em] text-subtle">Stock Limit</div>
                        <div className="mt-1 text-base font-semibold text-foreground">{formatCurrency(result.criteriaUsed.stockLimit)}</div>
                      </div>
                      <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                        <div className="text-xs uppercase tracking-[0.16em] text-subtle">Transaction Limit</div>
                        <div className="mt-1 text-base font-semibold text-foreground">{formatCurrency(result.criteriaUsed.transactionLimit)}</div>
                      </div>
                      <div className="rounded-lg border border-border/60 bg-background/60 p-3 sm:col-span-2 xl:col-span-1">
                        <div className="text-xs uppercase tracking-[0.16em] text-subtle">
                          {result.criteriaUsed.sellTargetReference?.source ?? "Sell Target"}
                        </div>
                        <div className="mt-1 text-base font-semibold text-foreground">
                          {result.criteriaUsed.sellTargetReference?.summary ?? "Unavailable"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="text-foreground">
                        Considered: <span className="text-subtle">{result.criteriaUsed.considered?.join(", ")}</span>
                      </div>
                      <div className="text-foreground">
                        Not considered: <span className="text-subtle">{result.criteriaUsed.notConsidered?.join(", ")}</span>
                      </div>
                    </div>
                  </div>
                ) : null}
                ...
                <div className="space-y-3 rounded-xl border border-border/70 bg-background/50 p-4">
                  <div className="text-xs uppercase tracking-[0.18em] text-subtle">Trade History</div>
                  <div className="space-y-2">
                    {result.result.trades.slice(0, 20).map((trade, index) => (
                      <div key={`${trade.date}-${trade.action}-${index}`} className="rounded-lg border border-border/60 bg-background/60 px-3 py-2">
                        <div className="flex items-start justify-between gap-3">
                          <div className={`text-sm font-semibold ${trade.action === "BUY" || trade.action === "ADD" ? "text-emerald-400" : "text-rose-400"}`}>
                            {trade.action}
                          </div>
                          <div className="text-xs text-subtle">{trade.date}</div>
                        </div>
                        <div className="mt-1 flex items-center justify-between gap-3 text-xs text-subtle">
                          <div>Price: {formatCurrency(trade.price)}</div>
                          {trade.profit != null ? (
                            <div className={metricTone(trade.profit)}>P/L: {formatCurrency(trade.profit)}</div>
                          ) : (
                            <div />
                          )}
                        </div>
                  ...
                  {result.result.trades.length > 20 ? (
                    <div className="text-xs text-subtle">... and {result.result.trades.length - 20} more trades</div>
                  ) : null}
                </div>
              </div>
            ) : (
            ...
            watchlistResult.ok && watchlistResult.result && watchlistResult.universe ? (
              <div className="space-y-4">
                ...
                <div className="grid gap-3 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
                  ...
                  <div className="rounded-xl border border-border/70 bg-background/50 p-4">
                    <div className="text-xs uppercase tracking-[0.18em] text-subtle">Settings Applied</div>
                    <div className="mt-2 text-sm text-foreground">{watchlistResult.universe.gating?.considered?.[0]}</div>
                    <div className="mt-3 text-sm text-foreground">Considered</div>
                    <div className="mt-1 text-sm text-subtle">{watchlistResult.universe.gating?.considered?.slice(1).join(" • ")}</div>
                    <div className="mt-3 text-sm text-foreground">Not considered</div>
