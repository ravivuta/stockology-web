/**
 * Risk–return score and recommendation rules aligned with iOS
 * `RecommendationEngine.compute` + `UnifiedDataStore.calculateScore` (Stockology/).
 */
...
export type IosStockInput = {
  ...
  movingAvg?: number;
  /** Shortlist / watchlist flags (from iOS sync or local defaults). */
...
export type IosRecOptions = {
  /** Daily closes, oldest → newest (matches iOS `closes`). */
  closes?: number[];
...
export function computeRiskReturnScore(stock: IosStockInput): number | undefined {
  ...
  if (at != null && at > 0 && lp > 0) {
    ...
    total += Math.min(Math.max((upsidePercent / 100) * 50, 0), 50);
  }

  const marketCap = stock.marketCap;
  if (marketCap != null && marketCap > 0) {
    let capScore = 0;
    if (marketCap >= 200_000_000_000) capScore = 20;
    else if (marketCap >= 50_000_000_000) capScore = 15;
    else if (marketCap >= 10_000_000_000) capScore = 10;
    else if (marketCap >= 1_000_000_000) capScore = 5;
    total += capScore;
  }
...
function generateReduceComment(reduceQty: number): string {
  return `Consider diversifying. Reduce your holding size by selling some stocks - sell ${reduceQty} shares to reduce cost basis`;
}

function generateSellComment(targetPrice: number): string {
  return `Price above target sell price $${targetPrice.toFixed(2)}`;
}
...
export function computeRecommendationFactors(stock: IosStockInput, rec: IosRecOut, skipWashSale: boolean): RecFactor[] {
  const factors: RecFactor[] = [];
  const currentPrice = stock.lastPrice ?? 0;
  const isETF = stock.isETF === true;
  const score = stock.score ?? 0;
  const isShortlisted =
    stock.isShortlisted ?? (stock.quantity > 0 || stock.isETF === true);
  ...
  switch (action) {
    ...
    case "WAIT_ADD":
      ...
      break;
    case "SELL": {
      const target = stock.analystTarget;
      if (target != null && target > 0) {
        factors.push({
          label: "Price at/above analyst target",
          detail: `$${currentPrice.toFixed(2)} ≥ $${target.toFixed(2)}`,
          passes: currentPrice >= target,
        });
      }
      break;
    }
  ...
  const isBuyAction = ["BUY", "ADD", "WAIT_BUY", "WAIT_ADD"].includes(action);
  if (isBuyAction) {
    if (!isETF) {
      factors.push({
        label: "Score ≥ 50",
    ...
    factors.push({
      ...
      detail: skipWashSale ? "Not evaluated on web" : "See iOS for restrictions",
      passes: skipWashSale,
    });
  }
...
export function scoreBreakdownRows(stock: IosStockInput): {
  ...
  return {
    ...
    analystPoints: `${analystPoints.toFixed(1)}/30`,
    upsideLine: upsidePct != null ? `${upsidePct.toFixed(1)}%` : "—",
    upsidePoints: `${upsidePoints.toFixed(1)}/50`,
...
export function computeIosRecommendation(stock: IosStockInput, options: IosRecOptions = {}): IosRecOut {
  const { closes = [], etfProfitTargetPercent = 50, stockProfitTargetPercent = 50, relaxScoreRequirement = false } =
    options;
  ...
  if (currentPrice <= 0) {
    return {
      ...
      comments: "Unable to fetch current price. Refresh quotes from Portfolio or Dashboard.",
      nextBuyPrice: 0,
      movingAvg: 0,
      expectedReturnPct: 0,
    };
  ...
  if (pre != null && pre > 0) {
    ...
    if (closes.length < minClosesRequired) {
      const nextBuyPrice = stock.quantity === 0 ? stock.averageCost || currentPrice : stock.averageCost || currentPrice;
      return {
        action: stock.quantity === 0 ? "WAIT_BUY" : "WAIT_ADD",
        comments: `Insufficient historical data (${closes.length}/${minClosesRequired} days). Open Snapshot after history loads, or refresh from iOS.`,
        nextBuyPrice,
        movingAvg: 0,
        expectedReturnPct: 0,
      };
    ...
    actualSmaPeriod =
      closes.length >= shortSMAPeriod ? shortSMAPeriod : Math.max(25, closes.length);
  ...
  const rawBuyFactor =
    numberPurchases === 0 ? 1 : 1 - dynamicFactor / 100 - (5 * numberPurchases) / 100;
  const buyFactorSMA = Math.max(0.1, rawBuyFactor);

  let nextBuyPrice: number;
  if (numStock === 0 || avgPrice <= 0) {
    nextBuyPrice = movingAvg;
  } else {
    nextBuyPrice = avgPrice * buyFactorSMA;
  }

  const analystT = stock.analystTarget;
  let targetPrice: number | undefined;
  if (analystT != null && analystT > 0) {
    targetPrice = analystT;
  } else if (stock.isETF) {
  ...
  if (numStock > 0 && targetPrice != null && currentPrice >= targetPrice) {
    return {
      action: "SELL",
      comments: generateSellComment(targetPrice),
      nextBuyPrice,
      movingAvg,
      expectedReturnPct,
    };
  ...
  if (
    ...
    costBasis < 2 * stockLimit &&
    currentPrice < transactionLimit &&
    gateReturnAndScore
  ) {
    if (costBasis < 2 * stockLimit) {
      if (!relaxScoreRequirement && stock.isETF !== true) {
        ...
        if (ai != null && ai > 0 && ai < 50) {
          ...
          return {
            ...
            comments: `AI input (news digest) is ${label} (${ai.toFixed(0)}/100) — overriding buy recommendation`,
            nextBuyPrice,
            movingAvg,
            expectedReturnPct,
          };
      ...
      if (numStock === 0) {
        const qty = Math.round(transactionLimit / currentPrice);
        return {
          action: "BUY",
          comments: `Buy ${qty} shares. Current price is below ${actualSmaPeriod}-day moving avg $${movingAvg.toFixed(2)}`,
          nextBuyPrice,
          movingAvg,
          expectedReturnPct,
        };
      ...
      const sc = metricScore;
      if (sc < 50) {
        return {
          action: "WAIT_ADD",
          comments: `Score ${sc.toFixed(0)}/100 below threshold (50) for adding to position. Wait for improvement.`,
          nextBuyPrice,
          movingAvg,
          expectedReturnPct,
        };
      ...
      return {
        action: "ADD",
        comments: `Add ${addQty} shares. Current price is below next target buy price $${nextBuyPrice.toFixed(2)}`,
        nextBuyPrice,
        movingAvg,
        expectedReturnPct,
      };
    ...
    return {
      action: "WAIT_ADD",
      comments: "Cost basis at limit. Wait before adding more.",
      nextBuyPrice,
      movingAvg,
      expectedReturnPct,
    };
  ...
  if (numStock > 0 && costBasis > stockLimit && reduceQty > 0 && currentPrice > movingAvg) {
    return {
      action: "REDUCE",
      comments: generateReduceComment(reduceQty),
      nextBuyPrice,
      movingAvg,
      expectedReturnPct,
    };
  ...
  if (numStock === 0) {
    return {
      action: "WAIT_BUY",
      comments: `Wait for price to fall below ${shortSMAPeriod}-day moving average: $${movingAvg.toFixed(2)}`,
      nextBuyPrice,
      movingAvg,
      expectedReturnPct,
    };
  ...
  if (costBasis >= 2 * stockLimit) {
    return {
      action: "WAIT_ADD",
      comments:
        "Current exposure has reached the 2x Stock Limit averaging-down cap. Wait to reduce position on strength before adding more.",
      nextBuyPrice,
      movingAvg,
      expectedReturnPct,
    };
  ...
  return {
    action: "WAIT_ADD",
    comments: `Add more shares @next target buy: $${nextBuyPrice.toFixed(2)}`,
    nextBuyPrice,
    movingAvg,
    expectedReturnPct,
  };
